<?php
	...
	//用户名长度不能小于3
	if(strlen($user) < 3){
		echo '<script>alert("用户名长度不能小于3");window.location.href="../register.php";</script>';
		exit;
	}

	//两次密码得一致  加密写入到表中
	if($pwd != $repwd){
		echo '<script>alert("两次密码不一致，请重新输入");window.location.href="../register.php";</script>';
		exit;
	}else{
		$pass = md5($pwd);
	}
	...
	if(!preg_match($pattern,$email)){
		...
		echo '<script>alert("邮箱格式不正确");window.location.href="../register.php";</script>';
		exit;
	}
	...
	if( $yzres && mysql_num_rows($yzres)){
		echo '<script>alert("用户名存在，请重新填写用户名！");window.location.href="../register.php";</script>';
		exit;	
	...
	if($res && mysql_affected_rows()){
		...
		echo '<script>alert("添加失败");window.location.href="../register.php";</script>';
		exit;
	}
