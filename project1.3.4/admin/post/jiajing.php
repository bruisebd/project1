<?php
	...
	if($jinghua == 1){ //sta 1 已经高亮的帖子   0 
		...
		if($res){
			...
			echo "<script>alert('取消加精显示');window.location.href='./post_list.php?&cid={$_GET['cid']}';</script>";
		...
		if($res){
			echo "<script>alert('加精显示成功');window.location.href='./post_list.php?&cid={$_GET['cid']}';</script>";
