<?php
	...
	if($gaoliang == 1){ //sta 1 已经高亮的帖子   0 
		...
		if($res){
			...
			echo "<script>alert('取消高亮显示');window.location.href='./post_list.php?&cid={$_GET['cid']}';</script>";
		...
		if($res){
			echo "<script>alert('高亮显示成功');window.location.href='./post_list.php?&cid={$_GET['cid']}';</script>";
